...
export default function BandPage() {
  ...
  const loadComments = async (releaseId: string) => {
    const { data } = await supabase
      .from('comments')
      .select('id, content, created_at, user_id, profiles(username)')
      .eq('release_id', releaseId)
      .order('created_at', { ascending: true })
    if (data) setComments(prev => ({ ...prev, [releaseId]: data as any }))
  }
  ...
  const handleSubmitComment = async (releaseId: string) => {
    if (!currentUser) return
    const content = commentInputs[releaseId]?.trim()
    if (!content) return
    setSubmittingComment(releaseId)

    const { data } = await supabase
      .from('comments')
      .insert({ release_id: releaseId, user_id: currentUser, content })
      .select('id, content, created_at, user_id, profiles(username)')
      .single()
    ...
    if (data) {
      setComments(prev => ({
        ...prev,
        [releaseId]: [...(prev[releaseId] || []), data as any]
      }))
      setCommentInputs(prev => ({ ...prev, [releaseId]: '' }))
    }
  ...
  const handleDeleteComment = async (releaseId: string, commentId: string) => {
    await supabase.from('comments').delete().eq('id', commentId)
    setComments(prev => ({
      ...prev,
      [releaseId]: prev[releaseId].filter(c => c.id !== commentId)
    }))
  }
                  ...
                  <div key={release.id} className="border border-zinc-800 rounded-xl overflow-hidden">
                    <button
                      onClick={() => {
                        ...
                        if (isOpening) {
                          if (!comments[release.id]) loadComments(release.id)
                        ...
                        <div className="border-t border-zinc-800">
                          <div className="px-4 py-4">
                            <p className="text-xs uppercase tracking-widest text-zinc-500 mb-4">
                              Comments ({(comments[release.id] || []).length})
                            </p>

                            {/* Comment list */}
                            {(comments[release.id] || []).length === 0 ? (
                              <p className="text-xs text-zinc-700 mb-4">No comments yet. Be the first!</p>
                            ) : (
                              <div className="flex flex-col gap-3 mb-4">
                                {(comments[release.id] || []).map(comment => (
                                  <div key={comment.id} className="flex gap-3 group">
                                    <div className="w-7 h-7 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-xs shrink-0 font-bold text-zinc-500">
                                    ...
                                    <div className="flex-1">
                                      <div className="flex items-baseline gap-2">
                                        ...
                                        {comment.user_id === currentUser && (
                                          <button
                                            onClick={() => handleDeleteComment(release.id, comment.id)}
                                            className="text-xs text-zinc-800 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100 ml-auto">
                                            Delete
                                          </button>
                                        )}
                                      </div>
                            ...
                            {currentUser ? (
                              <div className="flex gap-2">
                                <input
                                  type="text"
                                  value={commentInputs[release.id] || ''}
                                  onChange={e => setCommentInputs(prev => ({ ...prev, [release.id]: e.target.value }))}
                                  onKeyDown={e => e.key === 'Enter' && handleSubmitComment(release.id)}
                                  placeholder="Add a comment..."
                                  className="flex-1 bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-red-500 transition-colors"
                                />
                                <button
                                  onClick={() => handleSubmitComment(release.id)}
                                  disabled={submittingComment === release.id}
                                  className="bg-red-600 hover:bg-red-700 disabled:bg-zinc-700 text-white text-xs font-bold uppercase tracking-widest px-4 py-2 rounded-lg transition-colors">
                                  {submittingComment === release.id ? '...' : 'Post'}
                                </button>
                              </div>
                            ) : (
                              <p className="text-xs text-zinc-600">
                                <Link href="/login" className="text-red-500 hover:text-red-400">Login</Link> to leave a comment
                              </p>
                            )}
                          </div>
                        </div>
